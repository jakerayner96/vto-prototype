...
- Daily usage (5 try-ons per day) is communicated from the first generation, not just at the limit (gap flagged independently by Scott and Olivia). Quiet counter on the loading screen (grey line under the caption) and result screen (dark translucent pill, top-left opposite the rail): "N of 5 today". On the fifth try both switch to a warning state, "This is your last try-on today", in the expiry-chip treatment (white bg, #d33f3f). Each startGenerating() spends a try; photo-check analysing runs do not count and hide the counter (photomode). Tapping the result pill is a hidden review target cycling the count 1-5 so the last-try state demos without five generations. This pattern is proposed, not in the Figma file; the hard Daily Usage Limits Exceeded modal remains unbuilt.
